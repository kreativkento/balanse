...
--
-- Safe to re-run: ensures role = dev and a profiles_staff row exists.
...
begin
  if exists (select 1 from auth.users where email = v_email) then
    ...
    select coalesce(nullif(trim(name), ''), 'Dev')
    into v_name
    ...
    if v_name is not null then
      ...
      values (v_account_id, v_name, v_name, 'Dev')
      ...
      insert into public.profiles_staff (account_id, name, display_name, staff_type)
      values (v_account_id, 'Dev', 'Dev', 'Dev');
    end if;

    raise notice 'Account already exists for %. Role set to dev and profiles_staff ensured.', v_email;
    return;
  end if;
  ...
  insert into auth.users (
    ...
    jsonb_build_object('first_name', 'Studio', 'last_name', 'Dev'),
    now(),
    now(),
    '',
    '',
  ...
  if v_account_id is null then
    raise exception 'Failed to create public.accounts for dev.';
  end if;
