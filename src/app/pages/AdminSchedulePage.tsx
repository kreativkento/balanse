      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {activeTab === 'requests' && (
          <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-[2fr_1.8fr_1fr_0.8fr_1.3fr_1fr_auto] gap-4 px-6 py-3 border-b border-[#D4CDB5]/40 bg-[#F8F3E8]/40">
            ...
            <div className="divide-y divide-[#D4CDB5]/30">
              {bookingReqs.map(req => (
                <div key={req.id} className="grid grid-cols-[2fr_1.8fr_1fr_0.8fr_1.3fr_1fr_auto] gap-4 px-6 py-4 items-center hover:bg-[#F8F3E8]/50 transition-colors">
                  <div>
                    <p className="text-[#1E2A35] text-sm font-semibold">{req.student}</p>
                    <p className="text-[#B0A898] text-xs">{req.email}</p>
                  </div>
                  <div>
                    <p className="text-[#1E2A35] text-sm font-medium">{req.class}</p>
                    <p className="text-[#9A8E7E] text-xs">{req.date} · {req.time}</p>
                  </div>
                  <p className="text-[#5A5048] text-xs">{req.method}</p>
                  <p className="text-[#1E2A35] font-bold" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1rem' }}>₱{req.amount}</p>
                  <p className="text-[#8A7E6E] text-xs font-mono">{req.ref}</p>
                    ...
                    {req.status === 'pending' && (
                      rejectBId === req.id ? (
                        <div className="flex items-center gap-1 bg-red-50 border border-red-200 rounded-xl px-2 py-1.5">
                          ...
                          <button onClick={() => rejectBooking(req.id)} className="w-5 h-5 rounded-lg bg-red-500 text-white flex items-center justify-center"><Check size={10} /></button>
                          <button onClick={() => setRejectBId(null)} className="w-5 h-5 rounded-lg bg-white border border-[#D4CDB5] flex items-center justify-center text-[#8A7E6E]"><X size={10} /></button>
                        </div>
                      ) : (
                        <>
                          <button onClick={() => approveBooking(req.id)} className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white text-xs font-bold rounded-xl hover:bg-green-700 active:scale-95 transition-all"><Check size={11} /> Approve</button>
                          <button onClick={() => setRejectBId(req.id)} className="flex items-center gap-1 px-3 py-1.5 bg-red-50 text-red-600 border border-red-200 text-xs font-bold rounded-xl hover:bg-red-100 active:scale-95 transition-all"><X size={11} /> Reject</button>
                        </>
                      )
                    )}
                  </div>
        ...
        {activeTab === 'cancellations' && (
          <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-[2fr_1.8fr_1.2fr_1fr_auto] gap-4 px-6 py-3 border-b border-[#D4CDB5]/40 bg-[#F8F3E8]/40">
              {['Student', 'Class / Date', 'Notice', 'Status', ''].map(h => (
                <p key={h} className="text-[#8A7E6E] text-xs uppercase tracking-widest font-medium">{h}</p>
              ))}
            </div>
            <div className="divide-y divide-[#D4CDB5]/30">
              {cancelReqs.map(req => (
                <div key={req.id} className="grid grid-cols-[2fr_1.8fr_1.2fr_1fr_auto] gap-4 px-6 py-4 items-center hover:bg-[#F8F3E8]/50 transition-colors">
                  <div>
                    <p className="text-[#1E2A35] text-sm font-semibold">{req.student}</p>
                    <p className="text-[#B0A898] text-xs">{req.email}</p>
                  </div>
                  <div>
                    <p className="text-[#1E2A35] text-sm font-medium">{req.class}</p>
                    <p className="text-[#9A8E7E] text-xs">{req.date} · {req.time} · {req.coach}</p>
                  </div>
                  <div className="flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    {req.status === 'pending' && (
                      rejectCId === req.id ? (
                        <div className="flex items-center gap-1 bg-red-50 border border-red-200 rounded-xl px-2 py-1.5">
                          <span className="text-red-700 text-xs font-semibold">Reject?</span>
                          <button onClick={() => rejectCancel(req.id)} className="w-5 h-5 rounded-lg bg-red-500 text-white flex items-center justify-center"><Check size={10} /></button>
                          <button onClick={() => setRejectCId(null)} className="w-5 h-5 rounded-lg bg-white border border-[#D4CDB5] flex items-center justify-center text-[#8A7E6E]"><X size={10} /></button>
                        </div>
                      ) : (
                        <>
                          <button onClick={() => approveCancel(req.id)} className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white text-xs font-bold rounded-xl hover:bg-green-700 active:scale-95 transition-all"><Check size={11} /> Approve</button>
                          <button onClick={() => setRejectCId(req.id)} className="flex items-center gap-1 px-3 py-1.5 bg-red-50 text-red-600 border border-red-200 text-xs font-bold rounded-xl hover:bg-red-100 active:scale-95 transition-all"><X size={11} /> Reject</button>
                        </>
                      )
                    )}
                  </div>
