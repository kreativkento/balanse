...
type Category = 'Classes' | 'Studio' | 'Events' | 'Community';
...
const INITIAL_PHOTOS: GalleryPhoto[] = [
  { id: 1,  url: 'https://images.unsplash.com/photo-1767611120077-3697335ec748?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=800', caption: 'Yoga Morning Session with Coach Jodi',     category: 'Classes',   taggedStudents: ['Alex Johnson', 'Sofia Reyes'],   uploadedAt: 'Apr 7, 2026',  uploadedBy: 'Studio Admin' },
  { id: 2,  url: 'https://images.unsplash.com/photo-1637157216470-d92cd2edb2e8?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=800', caption: 'Mat Pilates – Core Control',               category: 'Classes',   taggedStudents: ['Maria Santos'],                  uploadedAt: 'Apr 6, 2026',  uploadedBy: 'Studio Admin' },
  { id: 3,  url: 'https://images.unsplash.com/photo-1686133368810-24f662f65cad?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=800', caption: 'Kickboxing Power with Coach Wolf',         category: 'Classes',   taggedStudents: ['Ryan Bautista'],                 uploadedAt: 'Apr 4, 2026',  uploadedBy: 'Studio Admin' },
  { id: 4,  url: 'https://images.unsplash.com/photo-1717500252780-036bfd89f810?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=800', caption: 'Animal Flow – April Community Session',    category: 'Community', taggedStudents: ['Camille Cruz', 'Lea Mendoza'],   uploadedAt: 'Apr 2, 2026',  uploadedBy: 'Studio Admin' },
  { id: 5,  url: 'https://images.unsplash.com/photo-1759352856072-985a4ddab82d?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=800', caption: 'Capoeira – Art in Motion',                 category: 'Events',    taggedStudents: [],                                uploadedAt: 'Apr 3, 2026',  uploadedBy: 'Studio Admin' },
  { id: 6,  url: 'https://images.unsplash.com/photo-1602827114685-efbb2717da9f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=800', caption: 'Circuit Training – Full Energy',           category: 'Classes',   taggedStudents: [],                                uploadedAt: 'Apr 1, 2026',  uploadedBy: 'Studio Admin' },
];
...
const CATEGORIES: Category[] = ['Classes', 'Studio', 'Events', 'Community'];
...
function PhotoModal({
  ...
  const isEdit = initial !== null && initial.url !== '';
      ...
      <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="px-7 pt-7 pb-5 border-b border-[#D4CDB5]/50 flex items-center justify-between sticky top-0 bg-white z-10">
          <h3 className="text-[#1E2A35]" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.5rem', letterSpacing: '0.05em' }}>
            {isEdit ? 'Edit Post' : 'Post to Gallery'}
          </h3>
        ...
        <div className="px-7 py-6 flex flex-col gap-4">
          {/* URL */}
          <div>
            <label className="block text-[#8A7E6E] text-xs uppercase tracking-widest mb-1.5">Photo URL <span className="text-red-400">*</span></label>
            <input type="text" value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))} placeholder="https://…" className={INP} />
            {form.url && (
              <div className="mt-2 rounded-xl overflow-hidden border border-[#D4CDB5]/60 h-32">
          ...
          {/* Caption */}
          <div>
            <label className="block text-[#8A7E6E] text-xs uppercase tracking-widest mb-1.5">Caption <span className="text-red-400">*</span></label>
            <textarea value={form.caption} onChange={e => setForm(f => ({ ...f, caption: e.target.value }))} rows={2} placeholder="Describe this photo…" className={INP + ' resize-none'} />
          </div>
          ...
          {/* Tag students */}
          <div>
            <label className="block text-[#8A7E6E] text-xs uppercase tracking-widest mb-2">Tag Students <span className="text-[#B0A898]">(optional)</span></label>
        ...
        <div className="px-7 pb-7 flex gap-3 sticky bottom-0 bg-white border-t border-[#D4CDB5]/40 pt-5">
          ...
          >
            {isEdit ? 'Save Changes' : 'Post Photo'}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-[#F8F3E8] min-h-screen">
      ...
      {confirmDeleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(30,42,53,0.5)', backdropFilter: 'blur(4px)' }}>
          <div className="bg-white rounded-3xl border border-red-200 shadow-2xl w-full max-w-sm p-8 text-center">
            <div className="w-14 h-14 rounded-2xl bg-red-50 border border-red-200 flex items-center justify-center mx-auto mb-4"><Trash2 size={22} className="text-red-500" /></div>
            <h3 className="text-[#1E2A35] mb-2" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.4rem', letterSpacing: '0.05em' }}>Delete This Photo?</h3>
            <p className="text-[#8A7E6E] text-sm mb-7 leading-relaxed">This photo will be permanently removed from the gallery.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDeleteId(null)} className="flex-1 py-3 rounded-full border border-[#D4CDB5]/70 text-[#8A7E6E] text-sm hover:bg-[#EDE8D8] transition-all">Cancel</button>
              <button onClick={() => handleDelete(confirmDeleteId)} className="flex-1 py-3 rounded-full bg-red-500 text-white hover:bg-red-600 active:scale-[0.97] transition-all text-sm font-semibold shadow-sm">Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        ...
        <div className="pt-6 pb-5 border-b border-[#D4CDB5]/60 flex items-center justify-between gap-4">
          ...
          <button
            onClick={openUpload}
            className="flex items-center gap-2 bg-[#1E2A35] text-white px-5 py-2.5 rounded-full hover:bg-[#263545] active:scale-[0.97] transition-all shadow-sm shrink-0"
            style={{ fontFamily: "'Bebas Neue', sans-serif", letterSpacing: '0.08em', fontSize: '0.9rem' }}
          >
            <Upload size={15} /> Post Photo
          </button>
        </div>
            ...
            {filtered.map(photo => (
              <div key={photo.id} className={`group bg-white rounded-2xl border border-[#D4CDB5]/60 shadow-sm overflow-hidden hover:shadow-md ${CARD_HOVER_GROW}`}>
                ...
                <div className="relative overflow-hidden">
                  ...
                  {/* Actions (only show for staff's own posts or all for prototype) */}
                  <div className="absolute top-2.5 right-2.5 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {confirmDeleteId === photo.id ? (
                      <>
                        <button onClick={() => handleDelete(photo.id)} className="w-7 h-7 rounded-xl bg-red-500 flex items-center justify-center text-white hover:bg-red-600 shadow-sm"><Check size={12} /></button>
                        <button onClick={() => setConfirmDeleteId(null)} className="w-7 h-7 rounded-xl bg-white/90 flex items-center justify-center text-[#8A7E6E] hover:bg-white shadow-sm"><X size={12} /></button>
                      </>
                    ) : (
                      <button
                ...
                <div className="px-4 py-3">
                  <p className="text-[#1E2A35] text-sm font-medium leading-snug mb-2 line-clamp-2">{photo.caption}</p>
                  ...
                  {photo.taggedStudents.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {photo.taggedStudents.slice(0, 3).map(s => (
                        <span key={s} className="text-[10px] bg-[#EDE8D8] text-[#5A5048] px-2 py-0.5 rounded-full">{s.split(' ')[0]}</span>
                      ))}
                      {photo.taggedStudents.length > 3 && (
                        <span className="text-[10px] bg-[#EDE8D8] text-[#5A5048] px-2 py-0.5 rounded-full">+{photo.taggedStudents.length - 3}</span>
                      )}
                    </div>
                  )}
                </div>
