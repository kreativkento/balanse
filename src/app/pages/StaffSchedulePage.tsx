...
const MEMBERSHIP_COLOR: Record<string, string> = {
  'Gold': 'bg-[#c49a3c]/12 text-[#a67f2e] border-[#c49a3c]/30',
              ...
              <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-sm p-5 sticky top-6">
                ...
                <div className="grid grid-cols-7 gap-y-0.5">
                  {grid.map((day, idx) => {
                    ...
                    return (
                      <button key={key} onClick={() => handleDateSelect(key)} className={`relative flex flex-col items-center justify-center rounded-xl py-1.5 mx-0.5 transition-all ${isSelected ? 'bg-[#1E2A35] text-white shadow-[0_3px_12px_rgba(30,42,53,0.3)]' : isToday ? 'bg-[#c49a3c]/12 text-[#a67f2e]' : 'text-[#1E2A35] hover:bg-[#F0EBE0] cursor-pointer'}`}>
                        <span className="text-sm leading-none mb-1">{day}</span>
