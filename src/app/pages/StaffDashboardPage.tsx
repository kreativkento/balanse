...
export default function StaffDashboardPage() {
  ...
  const toggleDay = (day: string) => setAvailability(a => ({ ...a, [day]: !a[day] }));
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        ...
        <div className="pt-6 pb-5 border-b border-[#D4CDB5]/60">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[#1E2A35]/10 border border-[#1E2A35]/20 rounded-full flex items-center justify-center">
                <span className="text-[#1E2A35] font-bold text-sm">{staffUser.name.split(' ').map(n => n[0]).join('').slice(0, 2)}</span>
              </div>
              {!showLogoutConfirm ? (
                <button
                ...
                <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
                  ...
                  <button onClick={handleLogout} className="text-xs font-bold text-red-600 hover:text-red-800 px-2 py-1 bg-white rounded-lg border border-red-200 transition-colors">Yes</button>
                  <button onClick={() => setShowLogoutConfirm(false)} className="text-xs text-[#8A7E6E] px-2 py-1 hover:bg-white rounded-lg transition-colors">Cancel</button>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 pb-10">
          ...
          <div className="md:col-span-3 flex flex-col gap-5">
            ...
            {/* My Pending Schedule Requests */}
            <div className="bg-white rounded-3xl border border-amber-200/50 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-amber-100 bg-amber-50/40 flex items-center justify-between">
          ...
          <div className="md:col-span-2 flex flex-col gap-4">
            ...
            {/* Quick Actions */}
            <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-[#D4CDB5]/50">
            ...
            <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-sm p-5">
              <div className="flex items-center justify-between mb-1">
                <p className="text-[#8A7E6E] text-xs uppercase tracking-widest">My Availability</p>
                <span className="text-[#B0A898] text-xs">Apr 13–19</span>
              </div>
              <p className="text-[#9A8E7E] text-xs mb-3 leading-relaxed">Mark days you're available for schedule assignment.</p>
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 py-2 rounded-xl border transition-all ${
                      availability[day]
                        ...
                        : 'bg-[#F8F3E8] border-[#D4CDB5]/60 text-[#9A8E7E] hover:border-[#C49A3C]/40'
                    }`}
                  >
                    <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '0.75rem', letterSpacing: '0.06em' }}>{day}</span>
                    <span className={`text-[10px] font-semibold ${availability[day] ? 'text-[#C49A3C]' : 'text-[#D4CDB5]'}`}>
                      {availability[day] ? '✓' : '—'}
                    </span>
                  </button>
                ))}
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-[#1E2A35] text-white text-xs hover:bg-[#263545] transition-all flex items-center justify-center gap-1.5"
            ...
            {/* Staff Info */}
            <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-sm p-5">
              <p className="text-[#8A7E6E] text-xs uppercase tracking-widest mb-3">Logged In As</p>
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-2xl bg-[#1E2A35]/08 border border-[#1E2A35]/15 flex items-center justify-center">
                  <span className="text-[#1E2A35] font-bold">{staffUser.name.split(' ').map(n => n[0]).join('').slice(0, 2)}</span>
                </div>
                <div>
                  <p className="text-[#1E2A35] font-semibold text-sm">{staffUser.name}</p>
                  <p className="text-[#8A7E6E] text-xs">{staffUser.email}</p>
                </div>
              </div>
              <div className="mt-3 pt-3 border-t border-[#D4CDB5]/40 flex items-center justify-between">
                <span className="text-[#8A7E6E] text-xs">Role</span>
                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${staffUser.role === 'Administrator' ? 'bg-[#3A4A5A]/10 text-[#3A4A5A]' : 'bg-[#C49A3C]/12 text-[#A67E2A]'}`}>
                  {staffUser.role}
                </span>
              </div>
            </div>
            ...
            {/* Today at a Glance */}
            <div className="bg-white rounded-3xl border border-[#D4CDB5]/60 shadow-sm p-5">
              <p className="text-[#8A7E6E] text-xs uppercase tracking-widest mb-3">Today at a Glance</p>
